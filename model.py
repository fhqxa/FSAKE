...
class GraphPool(nn.Module):
    ...
    def forward(self, A, X):
        ...
        for i in range(batch):
            ...
            A_cpu = A[i].cpu()
            A_cpu = A_cpu.detach().numpy()
            A_numpy.append(A_cpu)
            ed = sp.coo_matrix(A_numpy[i])
            indices = np.vstack((ed.row, ed.col))
            edge_index = torch.LongTensor(indices).to(X.device)
            edge_weight = torch.FloatTensor(ed.data).to(X.device)
            scores = self.conv(X[i], edge_index, edge_weight)
